...
public struct PolygonGate : Gate {
  ...
  public static var maxDispatchedElementCount = 65536
  ...
  public func masking(_ population: Population) -> Population? {
    ...
    let eventCountPerIteration =
      PolygonGate.maxDispatchedElementCount / vertices.count
    let iterations =
      (x.count + eventCountPerIteration - 1) / eventCountPerIteration

    let queue = DispatchQueue(
      label: "ca.diploid.FlowKit.PolygonGate",
      attributes: .serial
    )
    DispatchQueue.concurrentPerform(iterations: iterations) { i in
      ...
      let count: Int
      if i + 1 < iterations {
        count = eventCountPerIteration
      } else {
        let remainder = x.count % eventCountPerIteration
        count = remainder == 0 ? eventCountPerIteration : remainder
      }

      var r = [UInt8](repeating: 1, count: count)
      outer: for idx in offset..<(offset + count) {
        ...
        r[idx - offset] = UInt8(intersections % 2)
      }
      queue.async {
        result.replaceSubrange(offset..<(offset + count), with: r)
      }
    }
    //TODO: barrier
